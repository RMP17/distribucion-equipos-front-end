            ...
            <v-card>
                <v-card-title class="grey lighten-2">
                    <v-layout align-center justify-space-between fill-height>
                        <span v-if="!modeEdit" class=title>Nueva Profesión</span>
                        <span v-else class=title>Editar Profesión</span>
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            create() {
                axios.post(this.$urlApi.resourcesProfesion,
                    this.profesion
                ).then(response => {
            ...
            update() {
                axios.put(this.$urlApi.resourcesProfesion + '/' + this.profesion.id,
                    this.profesion
                ).then(response => {
                    Object.assign(this.editData, this.profesion);
                    this.$toastr('success', '', 'TAREA REALIZADA CON ÉXITO');
                    this.dialog = false;
                    this.$nextTick(() => {
                        this.profesion = new Profesion();
            ...
            openDialog() {
                Object.assign(this.profesion, this.editData);
                this.dialog = true;
            }
        }
    }
